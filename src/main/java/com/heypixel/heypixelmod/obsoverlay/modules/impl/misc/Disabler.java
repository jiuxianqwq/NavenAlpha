...
public class Disabler extends Module {
    ...
    private final BooleanValue grimDuplicateRotPlace = ValueBuilder.create(this, "GrimDuplicateRotPlace")
            ...
            .build()
            .getBooleanValue();
    // [新增] Exponentially Small 绕过开关
    private final BooleanValue exponentiallySmall = ValueBuilder.create(this, "ExponentiallySmall")
    ...
    public void duplicateRotPlaceDisabler(EventPacket e) {
        if ((this.acaaimstep.getCurrentValue() || this.acaperfectrotation.getCurrentValue() || this.exponentiallySmall.getCurrentValue()) && e.getPacket() instanceof ServerboundMovePlayerPacket movePacket) {
            float currentYaw = getPacketYRot(movePacket);
            float currentPitch = getPacketXRot(movePacket);
            boolean modified = false;

            if (this.exponentiallySmall.getCurrentValue()) {
                float deltaYaw = Math.abs(currentYaw - this.lastYaw);
                float deltaPitch = Math.abs(currentPitch - this.lastPitch);

                if (deltaYaw > 0.0F && deltaYaw < 0.005F) {
                    currentYaw = this.lastYaw;
                    modified = true;
                    if(this.logging.getCurrentValue()) this.log("Fixed small yaw: " + deltaYaw);
                }

                if (deltaPitch > 0.0F && deltaPitch < 0.005F) {
                    currentPitch = this.lastPitch;
                    modified = true;
                    if(this.logging.getCurrentValue()) this.log("Fixed small pitch: " + deltaPitch);
                }
            }
