...
public class BlurUtils {
    ...
    public static void onRenderAfterWorld(EventRender2D e, float fps, int strengthIndex) {
        ...
        Naven.getInstance().getEventManager().call(new EventShader(e.stack(), EventType.BLUR));
        ...
        if (shaderDown == null) {
            ...
            for (int i = 0; i < fbos.length; i++) {
                if (fbos[i] == null) {
                    fbos[i] = new Framebuffer(1.0 / Math.pow(2.0, i));
                }
            }
        }
        ...
        if (blurTimer.delay(1000.0F / fps)) {
            PostProcessRenderer.beginRender(e.stack());
            renderToFbo(e.stack(), fbos[0], Minecraft.getInstance().getMainRenderTarget().getColorTextureId(), shaderDown, offset);

            for (int ix = 0; ix < iterations; ix++) {
                renderToFbo(e.stack(), fbos[ix + 1], fbos[ix].texture, shaderDown, offset);
            }

            for (int ix = iterations; ix >= 1; ix--) {
                renderToFbo(e.stack(), fbos[ix - 1], fbos[ix].texture, shaderUp, offset);
            }

            Minecraft.getInstance().getMainRenderTarget().bindWrite(true);
            blurTimer.reset();
            PostProcessRenderer.render(e.stack());
