...
public class ShadowUtils {
    ...
    public static void onRenderAfterWorld(EventRender2D e, float fps) {
        ...
        if (shadowTimer.delay(1000.0F / fps)) {
            shouldFresh = true;
            shadowTimer.reset();
        }
        ...
        if (shouldFresh) {
            ...
            Naven.getInstance().getEventManager().call(new EventShader(e.stack(), EventType.SHADOW));
            render.unbind();
        }

        GL.enableBlend();
        shader.bind();
        shader.set("u_Size", window.getWidth(), window.getHeight());
