    ...
    public double render(Mesh mesh, String string, double x, double y, Color color, double scale, boolean shadow) {
        ...
        for (int i = 0; i < string.length(); i++) {
            ...
            if (cp == 167 && i + 1 < string.length()) {
                ...
                CharData c = this.charData[cp];
                mesh.quad(
                        mesh.vec2(x + (double) c.x0() * scale, y + (double) c.y0() * scale).vec2(c.u0(), c.v0()).color(currentColor).next(),
                        mesh.vec2(x + (double) c.x0() * scale, y + (double) c.y1() * scale).vec2(c.u0(), c.v1()).color(currentColor).next(),
                        mesh.vec2(x + (double) c.x1() * scale, y + (double) c.y1() * scale).vec2(c.u1(), c.v1()).color(currentColor).next(),
                        mesh.vec2(x + (double) c.x1() * scale, y + (double) c.y0() * scale).vec2(c.u1(), c.v0()).color(currentColor).next()
                );
